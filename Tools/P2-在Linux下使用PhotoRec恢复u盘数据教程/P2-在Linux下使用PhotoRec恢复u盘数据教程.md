...
### 一、发现误删，怎么处理U盘？
首先不要慌张，若U盘插在Linux上，且你有正在传输的文件，你需要先把正在传输的文件**立刻立刻立刻**取消传输，之后在终端中输入
```bash
lsblk -f
```
...
**注意:** 选择恢复文件夹时，**一定要选择除了被恢复磁盘上的文件夹**，不然会导致PhotoRec恢复已经恢复的文件造成循环，占用磁盘空间，这里推荐你存在/home/用户名(这个用户名替换成你自己的)
这个目录下，可以再在这个目录下建一个文件夹后选择它，整理更方便，或者你选择你要存储的文件夹位置，之后，按"C"(注意大写)保存该设置
